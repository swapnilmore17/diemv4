...
class VoteMsg:
    def __init__(self, vote_info, ledger_commit_info, high_commit_qc, sender):
        ...
        self.sender = sender
        self.signature = str(ledger_commit_info.commit_state_id) + \
            str(ledger_commit_info.vote_info_hash)
...
class BlockTree:
    ...
    def process_qc(self, qc):
        if qc.ledger_commit_info:
            ...
            self.high_commit_qc = max(
                qc.vote_info.round,
                self.high_commit_qc.vote_info.round)
    ...
    def process_vote(self, v, f, author):
        ...
        vote_idx = str(v.ledger_commit_info.commit_state_id) + \
            str(v.ledger_commit_info.vote_info_hash)
