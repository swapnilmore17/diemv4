...
class LeaderElection:
    def __init__(
        ...
        # reputation leaders; // Map from round numbers to leaders elected due
        # to the reputation scheme
        self.exclude_size = exclude_size
        # Map from round numbers to leaders elected due to the reputation
        # scheme
    ...
    def elect_reputation_leader(self, qc):
        ...
        while i < self.window_size or len(last_authors) < self.exclude_size:
            current_block = self.ledger_module.committed_block(
        ...
        return active_validators[int(random.seed(
            current_qc.vote_info.round) * len(active_validators))]
